...
public class TileController : MonoBehaviour
{
    ...
    private void OnMouseDown()
    {
        ...
        if (isSelected)
        {
            Debug.Log("Tile deselected: " + name);
            Deselect();
        }
        else
        {
            ...
            else
            {
                ...
                {
                    Debug.Log("Swapping tiles: " + name + " and " + previousSelected.name);
                    TileController otherTile = previousSelected;
                    previousSelected.Deselect();

                    SwapTile(otherTile, () => {
                        Debug.Log("Swap completed callback");
                        if (board.GetAllMatches().Count > 0)
                        {
                            Debug.Log("Match found after swap");
                            board.Process();
                        }
                        else
                        {
                            Debug.Log("No match found after swap, reverting");
    ...
    public IEnumerator MoveTilePosition(Vector2 targetPosition, System.Action onCompleted)
    {
        ...
        Debug.Log($"Starting MoveTilePosition for {name} from {startPosition} to {targetPosition}");
        ...
        Debug.Log($"Completed MoveTilePosition for {name} at {targetPosition}");
